...
await flow('A', async () => {
  ...
  // ⚠️ 發現：#college-id 有 HTML required，留空會被瀏覽器原生驗證擋下、submit 事件不觸發，
  //    程式內「id 留空自動 slugify」的路徑實際上到不了（friction，見回報）。此處填 id 續行。
  const idVal = await page.locator('#college-id').inputValue();
  if (!idVal) {
    log('⚠️', 'A', '新增學院表單：學院 ID 欄位 required＋留空 → 原生驗證擋下 submit，「留空自動衍生 id」死碼（第一次用的人會卡：按儲存沒反應，只有欄位氣泡）');
    await page.fill('#college-id', '測試學院');
  }
  ...
  await check('A', '儲存成功 toast「已儲存」', async () => {
    await page.locator('.toast', { hasText: '已儲存' }).waitFor({ timeout: 5000 });
  });
  await check('A', '學院列表出現「測試學院」', async () => {
    await page.locator('#admin-tab-content tr', { hasText: '測試學院' }).waitFor({ timeout: 3000 });
  });
  await shot(page, 'A-學院清單含新列');
});
